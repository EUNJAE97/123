...
public class EmperorSlime extends Slimes {
    int slimeHp;
    int slimeDamage;

    public EmperorSlime(int slimeDamage, int slimeHp) {
    ...
    public void finalattack(Hero hero) {
        if (getSlimeHp() <= 15 && getSlimeHp() >= 0) {
            System.out.println("몬스터가 자폭을 합니다!");
            hero.setHeroHp(hero.getHeroHp()-(slimeDamage + 10));
    ...
    public void halfDamageAttack(Hero hero) {
        ...
        if (getSlimeHp() <= 15){setSlimeHp(getSlimeHp()-100);}
    }
